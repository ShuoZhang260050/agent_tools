...
def test_read_pdf_basic(tmp_path):
    ...
    out = read_pdf.invoke({"path": str(p)})
    assert "第 1 页" in out
...
def test_read_pdf_not_found():
    out = read_pdf.invoke({"path": "nonexistent.pdf"})
    assert "文件不存在" in out
...
def test_read_pdf_page_range(tmp_path):
    ...
    out = read_pdf.invoke({"path": str(p), "page_start": 2, "page_end": 3})
    assert "第 2 页" in out
    assert "第 3 页" in out
...
def test_read_pdf_max_chars_truncation(tmp_path):
    ...
    out = read_pdf.invoke({"path": str(p), "max_chars": 8})
    assert "Hello" in out
